...
if [ -z "$JDK" -a -s "$HOME/.PyCharmCE2018.2/config/pycharm.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.PyCharmCE2018.2/config/pycharm.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PyCharmCE2018.2/config/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PyCharmCE2018.2/config/pycharm$BITS.vmoptions"
